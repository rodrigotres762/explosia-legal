...
# js/visual.js, js/portada.js o la escena, si no los visitantes que ya
# entraron se quedan con la copia vieja guardada en su navegador.
# Los cuatro llevan siempre el mismo numero.
set -e
cd "$(dirname "$0")"
actual=$(grep -o 'visual\.css?v=[0-9]*' index.html | head -1 | sed 's/.*v=//')
nueva=$((actual + 1))
for f in css/visual.css css/portada.css js/visual.js js/portada.js; do
